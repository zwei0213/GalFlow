...
2.  点击右上角的 **设置 (齿轮图标)**。
3.  **Step 1: 选择游戏文件夹**：选择包含 `voice/` 和 `script/` 子目录的游戏根文件夹。浏览器询问时请授予读取权限。
...
3. **连接**：在应用设置中点击 "Check" 测试连接。
4. **设置**：输入目标 Deck（牌组名称，如 `Default`）和 Model（卡片类型，如 `Basic`）。
5. **导出**：在背词界面的卡片右上角点击 "ANKI" 按钮，即可将当前单词、释义及上下文语音导出到 Anki。
